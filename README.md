...
- run your own provider from the same web interface
...
The app keeps sensitive work on the server. The browser shows the interface, but SSH access, TON daemon access, and command execution stay on the server side.
...
- upload files from your browser into local staging
...
The `My Provider` page is for running the provider on the current VPS.

It can:

- deploy a new provider
- import an existing provider key when moving or recovering a provider
- show whether the daemon is connected to the provider
...
- prepare a Tonkeeper start link or QR code
- prepare a Tonkeeper cancel link or QR code
...
TON daemon settings that can be overridden:
...
- `TON_REMOTE_BASE_DIR`
  Remote upload and staging area
- `TON_REMOTE_BAG_SOURCE_DIR`
...
- contract discovery is based on wallet address and TonAPI traces
- provider discovery depends on TonAPI
- daemon behavior can still vary between TON Storage builds
