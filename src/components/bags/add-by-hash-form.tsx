...
export function AddByHashForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/bags/add-by-hash", {
        method: "POST",
        headers: {
      ...
      setResult({
        ok: false,
        payload: error instanceof Error ? error.message : "Add-by-hash failed.",
      });
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <label className="block space-y-2 text-sm">
        <span className="font-medium">Bag hash</span>
        <input
          value={hash}
          onChange={(event) => setHash(event.target.value)}
          placeholder="64-character bag hash"
      ...
      >
        {isSubmitting ? "Submitting..." : "Add By Hash"}
      </button>
